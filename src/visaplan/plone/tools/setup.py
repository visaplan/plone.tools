...
def make_renamer(**kwargs):
    ...
    def rename_object(uid, oldid, newid, **kwargs):
        query = {'UID': uid}
        brains = catalog(query)

        oldtitles  = kwargs.pop('oldtitles', [])
        newtitle = kwargs.pop('newtitle', None)

        if kwargs:
            logger.error('UID %(uid)r: ignored arguments %(kwargs)r!', locals())

        if not brains:
            raise ValueError('UID %(UID)r not found!', query)
        elif brains[1:]:
            number = len(brains)
            raise ValueError('Too many hits for query %(query)r! (%(number)d)'
                             % locals())
        changes = 0
        o = None
        try:
            ...
            if newtitle:
                title = brain.Title
                if title == newtitle:
                    logger.info('New title %(newtitle)r of %(o)r already set',
                                locals())
                elif title in oldtitles:
                    logger.info('setting title of %(o)r to %(newtitle)r '
                                '(old: %(title)r)',
                                locals())
                    o.title = newtitle
                    changes += 1
                else:
                    logger.error('unexpected title %(title)r of %(o)r!', locals())

            current_id = brain.getId
            if not newid:
                logger.info('No ID change of %(o)r', locals())
            elif current_id == newid:
                logger.info('renaming of %(oldid)r to %(newid)r already done.',
                            locals())
            elif current_id == oldid:
                parent = o.aq_parent
                logger.info('renaming %(o)r from %(current_id)r to %(newid)r.'
                            ' This will take a while!',
                            locals())
                parent.manage_renameObject(oldid, newid)
                o = getattr(parent, newid)
                changes += 1
                logger.info('renaming of %(oldid)r to %(newid)r done.', locals())
            else:
                logger.error('unexpected ID %(current_id)r for uid %(uid)r, skipping.', locals())
        except Exception as e:
            logger.error('renaming of %(oldid)r to %(newid)r failed!\n%(e)r', locals())
            raise
        else:
            if changes:
                logger.info('reindexing %(o)r ...', locals())
                o.reindexObject()
